package com.ly.redisspring.chat.service;
...
public class ChatRoomService implements WebSocketHandler {
    ...
    public Mono<Void> handle(WebSocketSession webSocketSession) {
        ...
        RListReactive<String> list = this.client.getList("history:" + room, StringCodec.INSTANCE);// add list here to save the chat content in the list, so even you join a chat group later, you can get all the messages before
        // subscribe
        webSocketSession.receive()
                .map(WebSocketMessage::getPayloadAsText)
               // .flatMap(topic::publish)  //if you don't want to show all messages to new member of group, use this flatMap instead of following flatMap
        ...
        Flux<WebSocketMessage> flux = topic.getMessages(String.class)
                .startWith(list.iterator()) //add this to show all the messages in the list
